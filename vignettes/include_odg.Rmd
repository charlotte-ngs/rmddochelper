...
## Workflow
An author is writing a document and decides to include a graphic into that document. To be able to do that, this graphic has to be created first. One possibility to do that is to run `rmddochelper::use_odg_graphic` inside an R-code-chunk as shown below

```{r my_first_graphic, eval=FALSE}
rmddochelper::use_odg_graphic(ps_odg_file = "my_first_graphic.odg")
```

This call creates and odg-file in the directory where this rmd-file is located. The question is how we can decide the location of this rmd-document. One possibility is to use the `rprojroot` package. The function `thisfile()` returns the absolute path to this rmd-document as shown below.
