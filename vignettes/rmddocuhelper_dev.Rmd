...
gsub_templ_pattern <- function(ps_docu_template, pl_repl_info){
  ...
  for (sTag in names(pl_repl_info)){
    s_docu_result <- gsub(get_repl_tag(psTag = sTag), pl_repl_info[[sTag]], s_docu_result, fixed = TRUE)
  }
  return(s_docu_result)
}
### # output result of replacement
cat(gsub_templ_pattern(ps_docu_template = docu_template, pl_repl_info = repl_info))
```
