...
In many documents, graphics to be inserted are based on screenshots or other images which are not directly produced by one of the R-graphics systems. Then it would be convenient to have the command to include the graphic directly inserted into the Rmarkdown source file. This can be done by first inserting the R-chunk into the Rmarkdown source document and giving a label to that chunk which is the same as the filename (without extension) of the graphics file to be included. The creation function of the odg-graphic searches in the Rmarkdown source file after the label and inserts the command to include the graphics file.
...
Something similar can be done with RC or R6 classes where methods are called without parenthesis.
